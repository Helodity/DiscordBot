...
namespace DiscordBotRewrite.Quote
{
    public class QuoteModule
    {
        ...
        async Task TryQuote(DiscordClient client, MessageReactionAddEventArgs args)
        {
            ...
            if (data.EmojiAmount <= 0)
            {
                return;
            }

            //Already quoted, ignore the reaction
            if (Bot.Database.Table<Quote>().ToList().Any(x => (ulong)x.OriginalMessage == args.Message.Id))
            {
                return;
            }
            ...
            IReadOnlyList<DiscordUser> potentialReactions = await proper_message.GetReactionsAsync(GetEmoji(client, data));

            //Clean up the reactions to remove unwanted users (bots and the author)
            IEnumerable<DiscordUser> countedReactions = potentialReactions.Where(x => x.Id != proper_message.Author.Id && !x.IsBot);
            ...
            if (quoteReactions >= data.EmojiAmount)
            {
                ...
                DiscordEmbedBuilder quoteEmbed = new DiscordEmbedBuilder()
